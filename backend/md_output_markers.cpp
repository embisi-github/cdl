...
extern void output_markers_mask_clock_edge_dependents( c_model_descriptor *model, t_md_module *module, t_md_signal *clock, int edge, int set_mask, int clr_mask )
{
    ...
    while ((reference = model->reference_set_iterate(&iter))!=NULL)
    {
        ...
        else if (reference->type==md_reference_type_clock_edge) // This can happen if an input to a module is needed before clock edge 'X' and the input is derived from a state generated by clock edge 'Y', Y may or may not equal X
        {
            //fprintf(stderr,"Not quite expected clock edge '%s' %d ref %p - is this ref supposed to depend on clock %s edge %d\n",
            //        reference->data.signal->name, reference->edge, reference,
            //        clock->name,edge);
        }
        else
        {
            fprintf(stderr,"Unknown ref type %d for %p - is this ref supposed to depend on clock %s edge %d\n",reference->type,reference,clock->name,edge);
        }
    }
}
