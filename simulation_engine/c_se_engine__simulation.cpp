     ...
     for (clk=global_clock_list; clk; clk=clk->next_in_list)
     {
          ...
          for (posedge=0; posedge<2; posedge++)
          {
               ...
               for (efr=clk->clocks_list; efr; efr=efr->next_in_list)
               {
                     ...
                     */
                    if (posedge && efr->signal->data.clock.posedge_prepreclock_fn)
                    {
                        ...
                        se_engine_function_call_add( &clk->posedge.prepreclock, efr, efr->signal->data.clock.posedge_prepreclock_fn );
                    }
                    ...
                    if ((!posedge) && efr->signal->data.clock.negedge_prepreclock_fn)
                    {
                        SL_DEBUG( sl_debug_level_info, "c_se_engine::build_schedule", "adding negedge prepreclock '%s.%s' fn delay %d remainder %d", efr->signal->module_instance->name, efr->signal->name, clk->delay, clk->negedge_remainder);
                         se_engine_function_call_add( &clk->negedge.prepreclock, efr, efr->signal->data.clock.negedge_prepreclock_fn );
                    }
